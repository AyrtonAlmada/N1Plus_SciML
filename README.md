...
$$
p^* = {\arg\min}\limits_{p}\mathcal{L}(p).
$$
...
$$
(\alpha^{opt},\sigma^{opt})={\arg\min}\limits_{\alpha,\sigma}\frac{\left\|\theta^{\mathrm{EMT}}-\theta^{\mathrm{N1Plus}}(\alpha,\sigma)\right\|^2}{\left\|\theta^{\mathrm{EMT}}\right\|^2}.
$$
